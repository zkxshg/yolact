...
class MobileNetV1(nn.Module):
    def __init__(self, extra_args=[]):
        ...
        self.layer_to_block = {0: '_preconv.', 1: 'layers.0.', 2: 'layers.0.', 3: 'layers.1.', 4: 'layers.1.', 5: 'layers.2.', 6: 'layers.2.'}

        self._preconv = self.conv_bn(3, 32, 2)
        self._make_layer(2, [(32, 64, 1), (64, 128, 2)])
        self._make_layer(2, [(128, 128, 1), (128, 256, 2)])
        self._make_layer(2, [(256, 256, 1), (256, 512, 2)])
        self._make_layer(6, [(512, 512, 1), (512, 512, 1), (512, 512, 1), (512, 512, 1), (512, 512, 1), (512, 1024, 2)])

        self.backbone_modules = [m for m in self.modules() if isinstance(m, nn.Conv2d)]

    def _make_layer(self, i, cfg):
        """ Here one layer means a string of n blocks. """
        layer_list = []
        ...
        for idx in range(i):
            layer_list.append(
                self.conv_dw(cfg[idx][0], cfg[idx][1], cfg[idx][2])
            )
        self.channels.append(cfg[-1][1])
        self.layers.append(nn.Sequential(*layer_list))
    ...
    def transform_key(self, k):
        vals = k.split('.')
        if 'base_net' in k:
            block = self.layer_to_block.get(int(vals[1]), 'layers.3.')
            return block + vals[2] + '.' + vals[3]
        return k
    ...
    def add_layer(self, conv_channels=1024, downsample=2):
        """ Add a downsample layer to the backbone as per what SSD does. """
...
def construct_backbone1(cfg):
    """ Constructs a backbone given a backbone config object (see config.py). """

    backbone = EfficientNet.from_name('efficientnet-b0',override_params={'num_classes':2})
    #backbone = PeleeNet(num_classes=2)

    #input_var = torch.autograd.Variable(torch.Tensor(1,3,320,320))
    #backbone.forward(input_var)
    ...
    modelpara = torch.load('./weights/checkpoint.pth.tar')
